...
import { calculateRiskFactors } from './calculations';
import RiskAssessment from './RiskAssessment';
import Recommendations from './Recommendations';

export type CreosoteInputs = {
  fuelType: string;
  frequency: string;
  woodType: string;
...
export default function CreosoteForm() {
  const [inputs, setInputs] = useState<CreosoteInputs>({
    fuelType: '',
    frequency: '',
    woodType: '',
    moistureContent: '',
  ...
  const assessment = calculateRiskFactors(inputs);
  const showResults = Object.values(inputs).every(value => value !== '');
    ...
    <div className="space-y-8">
      ...
      {showResults && (
        <div className="space-y-6">
          ...
          <Recommendations assessment={assessment} inputs={inputs} />
        </div>
      )}
    </div>
  );
